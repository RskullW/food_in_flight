...
const CategoryMainPart = () => {
  ...
  const [productsWithCertainCategory, setAllProductsWithCertainCategory] = useState([]);
  ...
  useEffect(() => {
    const getProductsWithCertainCategory = async () => {
      try {
        ...
        if (productsResponse.status === 200) {
          ...
          setAllProductsWithCertainCategory(filteredProducts);
        } else {
    ...
    Promise.all([getProductsWithCertainCategory(), getCategory()]).then(() => {
      setTimeout(() => {
        setIsLoading(false);
      }, 1000)
    })
            ...
            isLoading ? (
              ...
              <>
                ...
                <Wrap justify="center" margin="20px 0px" p="5px">
                  {
                    productsWithCertainCategory.map((product) => (
                      <WrapItem
                        key={product.slug}
                      >
                        <Card
